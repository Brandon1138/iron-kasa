...
export default function IphonesPage() {
  ...
  const products = useMemo(() => getProductsByCategory('iphone'), []);
  ...
  return (
    <section className={`${styles.paddings} px-6 py-12 relative`}>
      ...
      <div className={`${styles.innerWidth} mx-auto flex flex-col items-center relative z-10`}>
        ...
        >
          <h1 className="font-bold text-3xl text-white">Reparații iPhone</h1>
          <motion.p
        ...
        >
          {products.map((product, index) => (
            <motion.div key={product.slug} variants={fadeIn('up', 'tween', index * 0.1, 1)} className="flex justify-center">
              <ServiceCard
                service={product}
                onClick={() => router.push(`/iphones/${product.slug}`)}
                isAnimationComplete
              />
            </motion.div>
          ))}
