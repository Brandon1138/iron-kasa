...
export async function generateStaticParams() {
  return getAllSlugs('iphone').map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: { slug: string } }) {
  const product = getProductBySlug('iphone', params.slug);
  if (!product) return { title: 'iPhone - Not found' };
  return {
    title: `Service ${product.title} | iPhone Doctor`,
    description: `Servicii pentru ${product.title}: ecran, baterie, camere și altele.`,
  };
}

export default function IphoneDetailPage({ params }: { params: { slug: string } }) {
  const product = getProductBySlug('iphone', params.slug);
    ...
    <section className={`${styles.paddings} px-6 py-12 relative`}>
      ...
      <div className={`${styles.innerWidth} mx-auto flex flex-col items-center relative z-10`}>
        <div className="w-full flex items-center justify-between mb-6">
          <Link href="/iphones" className="text-white hover:underline">← Înapoi la iPhone</Link>
        </div>
