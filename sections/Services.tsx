...
interface ServicesProps {
  id?: string;
  onServiceSelect?: (service: iPhoneServiceDetail) => void;
}

export default function Services({ id = 'services', onServiceSelect }: ServicesProps): JSX.Element {
  const [selectedCategory, setSelectedCategory] = useState<number>(0);
  const [selectedService, setSelectedService] = useState<iPhoneServiceDetail | null>(null);
  const [isAnimationComplete, setIsAnimationComplete] = useState<boolean>(false);
  const [hoveredCategoryId, setHoveredCategoryId] = useState<number | null>(null);

  const categoryTitles: readonly string[] = [
    'Reparații iPhone',
    'Reparații iPad',
    'Reparații MacBook',
    'Reparații iMac',
  ...
  const handleServiceSelect = (service: iPhoneServiceDetail) => {
  ...
  return (
    <section className={`${styles.paddings} px-6 py-12 relative`} id={id}>
      ...
      <div className={`${styles.innerWidth} mx-auto flex flex-col items-center relative z-10`}>
        ...
        <motion.div
          key={categoryTitles[selectedCategory]}
          variants={fadeIn('up', 'tween', 0.2, 1)}
          initial="hidden"
          animate="show"
        ...
        <motion.div
          ...
          "
        >
          {serviceDetails[selectedCategory].map((service) => (
            <motion.div
              key={service.title}
